...
export const getCart = (orderId) => (dispatch, getState) => {
  ...
  else axios.get(`/api/cart/${orderId}`).then(({data}) => {
...
export const modifyCartAPI = async (product, quantity) => {
  const res = await axios.post(`/api/cart/modify`, {orderId: store.getState().orders.order.id, productId: product.id, quantity});
